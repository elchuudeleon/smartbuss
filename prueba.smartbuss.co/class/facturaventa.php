...
class FacturaVenta extends Sql{
	...
	public function getSaldoClientesEmpresa($empresa){
		...
		$sql="SELECT * 
			from factura_venta
			WHERE estado != '3' and estado!=5 and idEmpresa = $empresa ";
	...
	public function getFacturaPendiente($fecha){	
		...
		WHERE fv.fechaVencimiento='$fecha' AND (fv.estado=1 or fv.estado=2 or fv.estado=4) ";
	...
	public function getFacturaPendienteDias($dias){	
		...
		$sql="SELECT *
		FROM factura_venta fv
		WHERE datediff(curdate(),fv.fechaVencimiento)='$dias' AND (fv.estado=1 or fv.estado=2 or fv.estado=4) ";
