...
for ($i=12; $i > 0; $i--) { 
	...
	if(!empty($aSituacion)){
		...
		foreach ($aSituacion as $key => $value) {
			if($value["tipo"]==1){
				$iSituacion["activo"]=$value["total"]; 
			}elseif($value["tipo"]==2){
				$iSituacion["pasivo"]=$value["total"]; 
			}else{
				$iSituacion["patrimonio"]=$value["total"]; 
			}
		}
...
// print_r($idEmpresa);
// print_r($fechaIndicador[0]);
// print_r($fechaIndicador[1]);
// print_r($totalActivoCorriente);
// print_r($totalPasivoCorriente);
